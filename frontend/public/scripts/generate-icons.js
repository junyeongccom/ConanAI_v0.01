...
async function generateIcons() {
  const svgBuffer = fs.readFileSync(path.join(__dirname, '../icon/icon.svg'));
  
  for (const { size, name } of sizes) {
    await sharp(svgBuffer)
      .resize(size, size)
      .png()
      .toFile(path.join(__dirname, `../icon/${name}.png`));
  }
  ...
  await sharp(svgBuffer)
    .resize(180, 180)
    .png()
    .toFile(path.join(__dirname, '../icon/apple-touch-icon.png'));
  ...
  await sharp(svgBuffer)
    ...
    .png()
    .toFile(path.join(__dirname, '../icon/maskable-icon.png'));
}
