...
docker build -t aws-develope-gateway:local ./gateway_service
k3d image import aws-develope-gateway:local -c $CLUSTER_NAME

# 2. DSDGen 서비스 빌드
echo "📦 DSDGen 서비스 빌드 중..."
docker build -t aws-develope-dsdgen:local ./dsdgen_service
k3d image import aws-develope-dsdgen:local -c $CLUSTER_NAME

# 3. DSDCheck 서비스 빌드
echo "📦 DSDCheck 서비스 빌드 중..."
docker build -t aws-develope-dsdcheck:local ./dsdcheck_service
k3d image import aws-develope-dsdcheck:local -c $CLUSTER_NAME

# 4. Frontend 빌드
echo "📦 Frontend 빌드 중..."
docker build -t aws-develope-frontend:local ./frotend
